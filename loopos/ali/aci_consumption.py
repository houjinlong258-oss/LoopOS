"""ACI result consumption helpers for the Agent Loop Interface.

This module owns the ``consume_aci_result`` machinery that maps an
...
* :mod:`loopos.ali.session` is the public surface: it re-exports
  :func:`consume_aci_result` for the convenience of callers that
  only want one import.
* :mod:`loopos.ali.aci_consumption` (this module) holds the actual
  mapping logic, the payload builder, the policy-denial classifier,
  and the state-aware filter.

Public helpers:

* :func:`consume_aci_result` -- consume an :class:`AgentCommandResult`
  and drive the FSM.

The following private helpers are exposed at module scope (with
leading underscores) so :mod:`loopos.ali.session` can compose them
if it needs to. They are not part of the public ALI surface and
should not be imported from outside the package.

* :func:`_extract_event_payload` -- build a structured event payload.
* :func:`_is_policy_denial` -- classify a blocked result as a policy
  denial vs a structural / validation block.
* :func:`_desired_event_sequence` -- data-only status -> event mapping.
* :func:`_valid_events_in_state` -- read valid events from the FSM
  table for a given state.
* :func:`_filter_for_state` -- drop events not valid in the current
  state.
"""
...
# Note: ``loopos.ali.session.apply_event`` is intentionally NOT
# imported at module top-level here. ``loopos.ali.session`` imports
# :func:`consume_aci_result` from this module, so importing
# ``apply_event`` at top-level would create a circular dependency.
# The consume helper uses the FSM directly (one extra trivial
# method call) to keep the dependency one-way.
...
def consume_aci_result(
    ...
    * The session must be in a state from which at least one event
    ...
    engine = fsm or DEFAULT_FSM
    for event in events:
        record = engine.apply(session, event, payload)
